...
colnames(hyper) <-c("target", "age", "sex", "on_thyroxine", "query_on_thyroxine",
                    ...
                    "TBG")
View(hyper)
levels(hyper$target) #  "hypothyroid" "negative" 
str(hyper)
...
print(prop.table(table(hyper$target))) ## Table of Proportions
...
# model using treebag
help("trainControl")
# traincontrol is used for cross validation 
...
summary(tbmodel)
# All minus Target
### names(trainSplit) != 'target' ---> FALSE  OR TRUE
